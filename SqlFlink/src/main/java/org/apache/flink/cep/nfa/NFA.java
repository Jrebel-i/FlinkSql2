 ...
 * org.apache.flink.cep.nfa.sharedbuffer.SharedBuffer buffer}, which is a memory-optimized data-structure exactly for this purpose. Events
...
public class NFA<T> {
    ...
    public void open(RuntimeContext cepRuntimeContext, Configuration conf) throws Exception {
        for (State<T> state : getStates()) {
            for (StateTransition<T> transition : state.getStateTransitions()) {
                org.apache.flink.cep.pattern.conditions.IterativeCondition condition = transition.getCondition();
    ...
    public Collection<Map<String, List<T>>> process(
            final org.apache.flink.cep.nfa.sharedbuffer.SharedBufferAccessor<T> sharedBufferAccessor,
            final NFAState nfaState,
            final T event,
            final long timestamp,
            final org.apache.flink.cep.nfa.aftermatch.AfterMatchSkipStrategy afterMatchSkipStrategy,
            final org.apache.flink.cep.time.TimerService timerService)
            throws Exception {
    ...
    private void processMatchesAccordingToSkipStrategy(
            org.apache.flink.cep.nfa.sharedbuffer.SharedBufferAccessor<T> sharedBufferAccessor,
            NFAState nfaState,
        ...
        if (earliestMatch != null) {
            ...
            while (earliestMatch != null
                ...
                List<Map<String, List<org.apache.flink.cep.nfa.sharedbuffer.EventId>>> matchedResult =
    ...
    private class EventWrapper implements AutoCloseable {
        ...
        private final org.apache.flink.cep.nfa.sharedbuffer.SharedBufferAccessor<T> sharedBufferAccessor;

        private org.apache.flink.cep.nfa.sharedbuffer.EventId eventId;

        EventWrapper(T event, long timestamp, org.apache.flink.cep.nfa.sharedbuffer.SharedBufferAccessor<T> sharedBufferAccessor) {
            this.event = event;
            this.timestamp = timestamp;
            this.sharedBufferAccessor = sharedBufferAccessor;
        }

        org.apache.flink.cep.nfa.sharedbuffer.EventId getEventId() throws Exception {
            if (eventId == null) {
     ...
     *       <ol>
     *         <li>IGNORE (links in {@link org.apache.flink.cep.nfa.sharedbuffer.SharedBuffer} will still point to the previous event)
     *             <ul>
     ...
     *                   state)
     *               <li>lock the entry in {@link org.apache.flink.cep.nfa.sharedbuffer.SharedBuffer} as it is needed in the created
     *                   branch
     *             </ul>
     *         <li>TAKE (links in {@link org.apache.flink.cep.nfa.sharedbuffer.SharedBuffer} will point to the current event)
     *             <ul>
     ...
     *   <li>Release the corresponding entries in {@link org.apache.flink.cep.nfa.sharedbuffer.SharedBuffer}.
     * </ol>
     *
        ...
        for (StateTransition<T> edge : edges) {
            switch (edge.getAction()) {
                ...
                case TAKE:
                    ...
                    final org.apache.flink.cep.nfa.sharedbuffer.NodeId previousEntry = computationState.getPreviousBufferEntry();
                    ...
                    final org.apache.flink.cep.nfa.sharedbuffer.NodeId newEntry =
                    ...
                    final org.apache.flink.cep.nfa.sharedbuffer.EventId startEventId;
                    if (isStartState(computationState)) {
    ...
    private void addComputationState(
            ...
            DeweyNumber version,
            long startTimestamp,
            org.apache.flink.cep.nfa.sharedbuffer.EventId startEventId)
            throws Exception {
    ...
    private boolean checkFilterCondition(
            ConditionContext context, org.apache.flink.cep.pattern.conditions.IterativeCondition<T> condition, T event) throws Exception {
     ...
     *
     * @param sharedBufferAccessor The accessor to {@link org.apache.flink.cep.nfa.sharedbuffer.SharedBuffer} from which to extract the
     *     matches
     ...
     * @throws Exception Thrown if the system cannot access the state.
     */
    private Map<String, List<org.apache.flink.cep.nfa.sharedbuffer.EventId>> extractCurrentMatches(
            final org.apache.flink.cep.nfa.sharedbuffer.SharedBufferAccessor<T> sharedBufferAccessor,
            final ComputationState computationState)
            throws Exception {
        if (computationState.getPreviousBufferEntry() == null) {
    ...
    private class ConditionContext implements IterativeCondition.Context<T> {
        ...
        private final org.apache.flink.cep.time.TimerService timerService;
        ...
        private org.apache.flink.cep.nfa.sharedbuffer.SharedBufferAccessor<T> sharedBufferAccessor;
    ...
    public static class NFASerializer<T> extends TypeSerializer<MigratedNFA<T>> {
        ...
        public MigratedNFA<T> deserialize(DataInputView source) throws IOException {
            ...
            Queue<ComputationState> computationStates =
                    MigrationUtils.deserializeComputationStates(sharedBuffer, eventSerializer, source);
