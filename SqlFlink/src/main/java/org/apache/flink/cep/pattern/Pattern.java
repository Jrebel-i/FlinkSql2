 ...
 *
 * <p>A pattern definition is used by {@link NFACompiler} to
 * create a {@link NFA}.
 *
 * <pre>{@code
...
public class Pattern<T, F extends T> {
    ...
    /**
     * A quantifier for the pattern. By default set to {@link Quantifier#one(Quantifier.ConsumingStrategy)}.
     */
    ...
    public static <X> Pattern<X, X> begin(
        ...
        return new Pattern<X, X>(name, null, Quantifier.ConsumingStrategy.STRICT, afterMatchSkipStrategy);
    }
    ...
    public Pattern<T, F> until(IterativeCondition<F> untilCondition) {
        ...
        if (this.untilCondition != null) {
            throw new org.apache.flink.cep.pattern.MalformedPatternException("Only one until condition can be applied.");
        }
    ...
    public Pattern<T, T> next(final String name) {
        return new Pattern<>(name, this, Quantifier.ConsumingStrategy.STRICT, afterMatchSkipStrategy);
    }
    ...
    public Pattern<T, T> followedBy(final String name) {
        return new Pattern<>(name, this, Quantifier.ConsumingStrategy.SKIP_TILL_NEXT, afterMatchSkipStrategy);
    }
    ...
    public Pattern<T, T> followedByAny(final String name) {
        return new Pattern<>(name, this, Quantifier.ConsumingStrategy.SKIP_TILL_ANY, afterMatchSkipStrategy);
    }
     ...
     * @throws org.apache.flink.cep.pattern.MalformedPatternException if the quantifier is not applicable to this pattern.
     */
     ...
     *
     * @return The same pattern with a {@link Quantifier#looping(Quantifier.ConsumingStrategy)} quantifier
     *     applied.
     * @throws org.apache.flink.cep.pattern.MalformedPatternException if the quantifier is not applicable to this pattern.
     */
    ...
    /**
     * Applicable only to {@link Quantifier#looping(Quantifier.ConsumingStrategy)} and {@link
     * Quantifier#times(Quantifier.ConsumingStrategy)} patterns, this option allows more flexibility to the
     * matching events.
     *
     ...
     * @return The first pattern of a pattern sequence
     */
    public static <T, F extends T> org.apache.flink.cep.pattern.GroupPattern<T, F> begin(
            final Pattern<T, F> group, final AfterMatchSkipStrategy afterMatchSkipStrategy) {
        return new org.apache.flink.cep.pattern.GroupPattern<>(null, group, Quantifier.ConsumingStrategy.STRICT, afterMatchSkipStrategy);
    }
     ...
     * @return the first pattern of a pattern sequence
     */
    public static <T, F extends T> org.apache.flink.cep.pattern.GroupPattern<T, F> begin(Pattern<T, F> group) {
        return new org.apache.flink.cep.pattern.GroupPattern<>(
     ...
     * @return A new pattern which is appended to this one
     */
    public org.apache.flink.cep.pattern.GroupPattern<T, F> followedBy(Pattern<T, F> group) {
        return new org.apache.flink.cep.pattern.GroupPattern<>(
                this, group, Quantifier.ConsumingStrategy.SKIP_TILL_NEXT, afterMatchSkipStrategy);
    }
     ...
     * @return A new pattern which is appended to this one
     */
    public org.apache.flink.cep.pattern.GroupPattern<T, F> followedByAny(Pattern<T, F> group) {
        return new org.apache.flink.cep.pattern.GroupPattern<>(
                this, group, Quantifier.ConsumingStrategy.SKIP_TILL_ANY, afterMatchSkipStrategy);
    }
     ...
     * @return A new pattern which is appended to this one
     */
    public org.apache.flink.cep.pattern.GroupPattern<T, F> next(Pattern<T, F> group) {
        return new org.apache.flink.cep.pattern.GroupPattern<>(this, group, Quantifier.ConsumingStrategy.STRICT, afterMatchSkipStrategy);
    }
    ...
    private void checkIfNoNotPattern() {
        if (quantifier.getConsumingStrategy() == Quantifier.ConsumingStrategy.NOT_FOLLOW
            ...
            throw new org.apache.flink.cep.pattern.MalformedPatternException("Option not applicable to NOT pattern");
        }
    }
    ...
    private void checkIfNoGroupPattern() {
        if (this instanceof GroupPattern) {
            throw new org.apache.flink.cep.pattern.MalformedPatternException("Option not applicable to group pattern");
        }
    }
