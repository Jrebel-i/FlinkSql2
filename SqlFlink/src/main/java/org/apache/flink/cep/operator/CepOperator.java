...
/**
 * CEP pattern operator for a keyed input stream. For each key, the operator creates a {@link org.apache.flink.cep.nfa.NFA}
...
public class CepOperator<IN, KEY, OUT>
        extends AbstractUdfStreamOperator<OUT, org.apache.flink.cep.functions.PatternProcessFunction<IN, OUT>>
    ...
    private transient ValueState<org.apache.flink.cep.nfa.NFAState> computationStates;
    private transient MapState<Long, List<IN>> elementQueueState;
    private transient SharedBuffer<IN> partialMatches;

    private transient InternalTimerService<VoidNamespace> timerService;

    private org.apache.flink.cep.nfa.NFA<IN> nfa;

    /** Comparator for secondary sorting. Primary sorting is always done on time. */
    private final org.apache.flink.cep.EventComparator<IN> comparator;
    ...
    public CepOperator(
            ...
            final org.apache.flink.cep.functions.PatternProcessFunction<IN, OUT> function,
                ...
                context.getKeyedStateStore()
                        .getState(
                                new ValueStateDescriptor<>(
                                        NFA_STATE_NAME, new org.apache.flink.cep.nfa.NFAStateSerializer()));
    ...
    public void processElement(StreamRecord<IN> element) throws Exception {
        if (isProcessingTime) {
            if (comparator == null) {
                ...
                org.apache.flink.cep.nfa.NFAState nfaState = getNFAState();
    ...
    public void onProcessingTime(InternalTimer<KEY, VoidNamespace> timer) throws Exception {
        ...
        // STEP 1
        PriorityQueue<Long> sortedTimestamps = getSortedTimestamps();
        org.apache.flink.cep.nfa.NFAState nfa = getNFAState();
    ...
    private org.apache.flink.cep.nfa.NFAState getNFAState() throws IOException {
        org.apache.flink.cep.nfa.NFAState nfaState = computationStates.value();
        return nfaState != null ? nfaState : nfa.createInitialNFAState();
    }

    private void updateNFA(org.apache.flink.cep.nfa.NFAState nfaState) throws IOException {
     ...
     */
    private void processEvent(org.apache.flink.cep.nfa.NFAState nfaState, IN event, long timestamp) throws Exception {
        if(needChange(event)){
            changeNFA(event);
            return;
        }
    ...
    private boolean needChange(IN element){
        //获取用户对象
        org.apache.flink.cep.functions.PatternProcessFunction<IN, OUT> userFunction = this.userFunction;
        return userFunction.getFlagNeedListern()&&userFunction.needChange(element);
    }

    public void changeNFA(IN element) throws Exception {
        Pattern pattern = this.userFunction.getNewPattern(element);
        boolean timeoutHandling = userFunction instanceof org.apache.flink.cep.functions.TimedOutPartialMatchHandler;
        NFACompiler.NFAFactory<IN> nfaFactory = NFACompiler.compileFactory(pattern, timeoutHandling);
        org.apache.flink.cep.nfa.NFA<IN> newNFA = nfaFactory.createNFA();//获取工厂类
        nfa.open(cepRuntimeContext,new Configuration());//这个地方为所有的边transition设置了cepRuntimeContext
        nfa=newNFA; //覆盖原先的
...
//		下一个可匹配状态选择
        ...
        for (org.apache.flink.cep.nfa.State<IN> state : nfa.getStatesMap().values()) { //获取NFA中的状态
            if(state.isStart()){
                //创建一个start状态
     ...
     */
    private void advanceTime(org.apache.flink.cep.nfa.NFAState nfaState, long timestamp) throws Exception {
        try (SharedBufferAccessor<IN> sharedBufferAccessor = partialMatches.getAccessor()) {
    ...
    private void processMatchedSequences(
        ...
        org.apache.flink.cep.functions.PatternProcessFunction<IN, OUT> function = getUserFunction();
    ...
    private void processTimedOutSequences(
        ...
        org.apache.flink.cep.functions.PatternProcessFunction<IN, OUT> function = getUserFunction();
        if (function instanceof org.apache.flink.cep.functions.TimedOutPartialMatchHandler) {

            @SuppressWarnings("unchecked")
            org.apache.flink.cep.functions.TimedOutPartialMatchHandler<IN> timeoutHandler =
    ...
    /**
     * Gives {@link org.apache.flink.cep.nfa.NFA} access to {@link InternalTimerService} and tells if {@link CepOperator}
    ...
    /**
     * Implementation of {@link org.apache.flink.cep.functions.PatternProcessFunction.Context}. Design to be instantiated once per
