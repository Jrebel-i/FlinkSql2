...
public class SavepointMetadata {
    ...
    public OperatorState getOperatorStateOfOperatorID(OperatorID id) throws IOException {
        OperatorID operatorID = id;

        OperatorStateSpec operatorState = operatorStateIndex.get(operatorID);
        if (operatorState == null || operatorState.isNewStateTransformation()) {
            throw new IOException("Savepoint does not contain state with operatorID " + operatorID.toHexString());
        }
