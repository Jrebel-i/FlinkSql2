...
public class ExistingSavepoint extends WritableSavepoint<ExistingSavepoint> {
    ...
    public <T> DataSource<T> readListStateOfOperatorID(OperatorID operatorID, String name, TypeInformation<T> typeInfo)
            throws IOException {
        OperatorState operatorState = metadata.getOperatorStateOfOperatorID(operatorID);
        ListStateDescriptor<T> descriptor = new ListStateDescriptor<>(name, typeInfo);
        ListStateInputFormat<T> inputFormat = new ListStateInputFormat<>(operatorState, descriptor);
        return env.createInput(inputFormat, typeInfo);
    }
    ...
    public <T> DataSource<T> readListStateOfOperatorID(OperatorID operatorID, String name, TypeInformation<T> typeInfo, TypeSerializer<T> serializer)
            throws IOException {
        OperatorState operatorState = metadata.getOperatorStateOfOperatorID(operatorID);
        ListStateDescriptor<T> descriptor = new ListStateDescriptor<>(name, serializer);
        ListStateInputFormat<T> inputFormat = new ListStateInputFormat<>(operatorState, descriptor);
    ...
    public <T> DataSource<T> readUnionStateOfOperatorID(OperatorID operatorID, String name, TypeInformation<T> typeInfo)
            throws IOException {
        OperatorState operatorState = metadata.getOperatorStateOfOperatorID(operatorID);
        ListStateDescriptor<T> descriptor = new ListStateDescriptor<>(name, typeInfo);
        UnionStateInputFormat<T> inputFormat =
    ...
    public <K, V> DataSource<Tuple2<K, V>> readBroadcastStateOfOperatorID(
