 ...
 *
 * <p>The entries in {@link SharedBuffer} are {@link org.apache.flink.cep.nfa.sharedbuffer.SharedBufferNode}. The shared buffer node
...
public class SharedBuffer<V> {
    ...
    private final MapState<NodeId, Lockable<org.apache.flink.cep.nfa.sharedbuffer.SharedBufferNode>> entries;
    ...
    private final Map<NodeId, Lockable<org.apache.flink.cep.nfa.sharedbuffer.SharedBufferNode>> entryCache = new HashMap<>();
    ...
    public void migrateOldState(
        ...
        stateBackend.applyToAllKeys(
                ...
                new MapStateDescriptor<>(
                        ...
                        new NodeId.NodeIdSerializer(),
                        new Lockable.LockableTypeSerializer<>(
                                new org.apache.flink.cep.nfa.sharedbuffer.SharedBufferNode.SharedBufferNodeSerializer())),
                (key, state) -> {
                    copyEntries(state);
    ...
    private void copyEntries(MapState<NodeId, Lockable<org.apache.flink.cep.nfa.sharedbuffer.SharedBufferNode>> state) throws Exception {
        state.entries()
                .forEach(
                        e -> {
                            try {
    ...
    private void lockPredecessorEdges(Map.Entry<NodeId, Lockable<org.apache.flink.cep.nfa.sharedbuffer.SharedBufferNode>> e) {
        org.apache.flink.cep.nfa.sharedbuffer.SharedBufferNode oldNode = e.getValue().getElement();
        oldNode.getEdges()
                .forEach(
                        edge -> {
    ...
    private void lockEdges(NodeId nodeId, DeweyNumber version) {
        ...
        try {
            org.apache.flink.cep.nfa.sharedbuffer.SharedBufferNode newNode = entries.get(nodeId).getElement();
            newNode.getEdges()
                    .forEach(
                            newEdge -> {
     ...
     */
    public org.apache.flink.cep.nfa.sharedbuffer.SharedBufferAccessor<V> getAccessor() {
     ...
     */
    void upsertEntry(NodeId nodeId, Lockable<org.apache.flink.cep.nfa.sharedbuffer.SharedBufferNode> entry) {
